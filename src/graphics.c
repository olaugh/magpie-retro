...
/* Sprite table is at 0xD800 (register 5 = 0x6C means 0x6C * 0x200 = 0xD800) */
#define SPRITE_TABLE_ADDR 0xD800

/* Sprite tiles for row labels: digits 0-9 at tiles 256-265 */
#define SPRITE_TILE_DIGIT_0 256

/* Set a sprite entry in the sprite table
 * Sprite format (8 bytes per sprite):
 *   Word 0: Y position + 128 (0 = offscreen)
 *   Word 1: size (bits 8-11), link to next sprite (bits 0-6)
 *   Word 2: priority/palette/flip/tile index
 *   Word 3: X position + 128
 */
static void set_sprite(int sprite_idx, int x, int y, int tile, int link) {
    uint16_t addr = SPRITE_TABLE_ADDR + sprite_idx * 8;
    vdp_set_vram_write(addr);
    VDP_DATA = y + 128;              /* Y position (128 = top of visible area) */
    VDP_DATA = (0 << 8) | link;      /* Size 1x1 (8x8), link to next */
    VDP_DATA = tile;                 /* Tile index, palette 0, no flip */
    VDP_DATA = x + 128;              /* X position (128 = left of visible area) */
}

/* Initialize sprite table - just terminate the list */
static void init_sprites(void) {
    /* Set sprite 0 to terminate the list (link=0, Y=0 means offscreen) */
    set_sprite(0, 0, -128, 0, 0);
}
...
/* Margin text tiles: ASCII 32-95 with 2-pixel left margin, at tiles 192-255 */
#define TILE_MARGIN_BASE 192
...
/* five-pixel font - 5x5 glyphs in 6x6 cells, padded to 8 rows */
...
/* Load a character tile into VRAM with foreground/background colors */
static void load_char_tile_colored(int tile_index, int char_code, uint8_t fg, uint8_t bg) {
    const uint8_t *glyph;

    if (char_code >= 32 && char_code < 128) {
        glyph = font_data[char_code - 32];
    } else {
        glyph = font_data[0]; /* Space for unknown */
    }
...
/* Load a character tile with left margin (glyph shifted right by 2 pixels) */
static void load_char_tile_margin(int tile_index, int char_code) {
    const uint8_t *glyph;
    ...
    for (int row = 0; row < 8; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            uint8_t color;
            if (col < 2) {
                color = 0;  /* 2-pixel left margin (transparent/black) */
            } else {
                /* Shifted glyph: col 2 reads bit 7, col 3 reads bit 6, etc. */
                color = (bits & (0x80 >> (col - 2))) ? 1 : 0;
            }
            pixels |= ((uint32_t)color << ((7 - col) * 4));
        }
...
/* Load a sprite tile - glyph is left-aligned, 6 pixels wide, rest transparent */
static void load_sprite_char_tile(int tile_index, int char_code) {
    const uint8_t *glyph;
    ...
    for (int row = 0; row < 8; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            uint8_t color;
            if (col < 6) {
                /* 6-pixel wide glyph area */
                color = (bits & (0x80 >> col)) ? 1 : 0;  /* White on transparent */
            } else {
                color = 0;  /* Transparent padding on right */
            }
            pixels |= ((uint32_t)color << ((7 - col) * 4));
        }
...
void init_tiles(void) {
    ...
    /* Load margin versions of ASCII 32-95 at tiles 192-255 (2-pixel left margin) */
    for (int i = 32; i < 96; i++) {
        load_char_tile_margin(TILE_MARGIN_BASE + (i - 32), i);
    }
    ...
    /* Sprite initialization disabled for debugging */
}
...
/* Draw a character using margin tile (2-pixel left margin) */
static void draw_char_margin(int x, int y, char c, int pal) {
    if (c >= 32 && c < 96) {
        put_tile(x, y, TILE_MARGIN_BASE + (c - 32), pal);
    } else if (c >= 96 && c < 128) {
        /* Lowercase and beyond: use regular tiles */
        put_tile(x, y, c, pal);
    }
}
...
/* Draw a string using margin tiles */
static void draw_string_margin(int x, int y, const char *str, int pal) {
    while (*str) {
        draw_char_margin(x++, y, *str++, pal);
    }
}
...
/* Row label sprite configuration - only 6 sprites for the "1" in rows 10-15 */
#define ROW_LABEL_SPRITE_START 0

/* Set up row label sprites - just the "1" tens digit for rows 10-15 */
static void setup_row_label_sprites(void) {
    for (int i = 0; i < 6; i++) {
        int row = 9 + i;  /* Rows 10-15 (indices 9-14) */
        int y_pixel = (BOARD_TOP + row) * 8;
        /* Position "1" sprite 6 pixels left of the ones digit tile at column 1 (x=8) */
        int x_pixel = 8 - 6 + 2;  /* = 4, gives 2-pixel gap before ones digit */
        set_sprite(i, x_pixel, y_pixel, SPRITE_TILE_DIGIT_0 + 1, i + 1);
    }
    /* Terminate sprite list */
    set_sprite(6, 0, -128, 0, 0);
}
...
void draw_board(const Board *board) {
    ...
    /* Draw row labels - tiles only for now (sprites disabled for debugging) */
    for (int row = 0; row < BOARD_DIM; row++) {
